...
% peteThe high reactivity of OH results in its atmospheric lifetime being short, of the order of 0.1 – 1 s (Heard and Pilling, 2003), meaning it is not transported over any significant distance before undergoing reaction. Thus, OH concentrations can be considered to be controlled entirely through in situ production and loss processes
% The primary source of OH in the troposphere is the photolysis of O3 at λ ≤ 340 nm (Matsumi et al., 2002), with a rate of photolysis of j(O1D)
% %

% KRtThrough a combination of field observations and laboratory studies, a series of additional OH and HO2 (an important OH reservoir species) production mechanisms have been identified as significant radical sources within the troposphere. However, as these mechanisms depend on the photolysis or reaction of species that exhibit more spatial variability in their concentrations than ozone and water vapour, their importance in determining local OH concentrations varies depending on local chemical composition. The photolysis of formaldehyde (HCHO) (R1.4); HONO (R1.5); and peroxides such as H2O2 (R1.6); and CH3OOH (R1.7) have all been identified as significant sources of OH and its reservoir species HO2, in certain, especially polluted, environment
...
The model uses the  Kinetic PreProcessor (KPP) to convert a chemical mechanim into the set of ordinary differential equations which can be solved using a suite of FORTRAN numberical integrators it provides \citep{kpp}. The Tropospheric and Ultraviolet (TUV) \citep{tuv} is used to calcualte the strenghts of different photolysis reactions for the mechanism. These are calculated at the start of a simulation and then predicted using cubic splines \citep{dsmaccgit}. This is the model setup that will be used to propagate the chemistry throughout this thesis. 
